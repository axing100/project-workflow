...
- 实现状态：✅ 已完成
- 验收状态：⛔ 已阻塞
- 证据：插件清单提升至0.5.1；设计、计划和交付记录已写明用户接受的Windows桌面宿主残余风险与发布授权；未重装个人插件、未删除分支
- 原因：v0.5.1发布机械校验已通过且用户接受残余风险；客户Windows 10/11桌面Codex新任务加载与native-agent宿主全流程仍未实机验收，不能标记完整产品验收通过
...
- 原批准计划只授权上述实现/测试与原生智能体委派；后续提交、推送和 Windows CI 授权已另行取得。2026-09-07，用户明确接受 Windows 10/11 桌面 Codex 宿主未实机验收的残余风险，并独立授权合并 `main`、创建和推送正式标签 `v0.5.1`；该例外不授权本机重装或删除分支，也不把 T04/T05 的桌面宿主验收标记为通过。

## 检查记录

- 2026-09-07：用户在明确获知“客户 Windows 10/11 桌面 Codex 新任务加载及 native-agent 宿主流程尚未实机验收”后回复“接受该残余风险，将 `95d9b54` 合并到 `main`，并创建、推送正式标签 `v0.5.1`？”。据此恢复 T06，仅发布已由五组 CI 验证的代码，并持续保留 T04/T05 验收阻塞与交付限制；不重装个人插件、不删除功能分支。
