...
Use the following planning handoff structure in the user's current conversation language:

1. Open with one short sentence stating the workflow profile, task title, and planning outcomes actually completed. Use the semantic template "Using Project Workflow's {profile} workflow, I have completed {planning outcomes} for {task title}." Localize the profile naturally: for example, LIGHT / STANDARD / FULL become 轻量 / 标准 / 完整 in Chinese and lightweight / standard / full in English. Name only outcomes supported by the generated documents; do not imply implementation or acceptance testing has completed.
2. Place the durable document links immediately after that sentence. Give each link its own paragraph, separated by a blank line, with the whole link emphasized: `**[Design: actual-filename.md](/absolute/path/actual-filename.md)**` and `**[Implementation plan: actual-filename.md](/absolute/path/actual-filename.md)**`. Translate the role label but preserve the actual filename, including its extension, exactly after the colon. Escape Markdown or encode link targets as needed for valid links, without changing the displayed filename. Link only to files actually created or updated. For LIGHT with an embedded design, show only the existing plan link and briefly identify its design section; do not invent a separate design file. Do not add emoji, manual file icons, headings, tables, or horizontal rules around the links; Codex controls native file-link rendering.
3. Summarize the scope and implementation approach in at most three short bullets. Keep each bullet focused on one topic and avoid combining feature requirements, coordination details, and recovery limitations into one long item. Preserve the applicable evidence, rollback, and delegation disclosures above, using a separate concise paragraph when needed. Report planning validation and material limitations separately from the feature summary, and only claim checks actually performed. State briefly that, after confirmation, execution will follow the approved plan and the main task will report the start, actionable exceptions, long-running phase heartbeats, and final result. Mention native agents in the Codex UI only when the plan actually uses them; do not promise renderer behavior controlled by the host.
4. End with one concise confirmation request using the natural-language feature title and localized revision wording, and stating that implementation follows confirmation. Do not repeat the request or introduce another approval prompt above it. Machine identifiers such as `plan_id` and `revision` may appear separately for traceability, but must not be presented as text the user has to copy to approve.

Localize the opening, link labels, summary, validation wording, and confirmation request together. These are semantic templates, not fixed Chinese or English prose: preserve the meaning without reproducing fixed wording verbatim. Adapt to the current task, workflow profile, and conversation language; keep actual filenames, code identifiers, and persisted enum values unchanged. This conversational localization does not extend the task-state renderer's supported locales, which remain `zh-CN` and `en-US` with the existing fallback behavior.

Use valid Markdown with blank lines between paragraphs and lists. For inline emphasis before punctuation, prefer `**Review focus**: ...` over delimiter placement that may expose literal asterisks. Keep the handoff concise and the document links prominent.
