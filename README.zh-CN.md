...
Project Workflow 是一个面向复杂软件工程交付的 Codex 工作流。它将设计、实施、验证和交付证据持久化到仓库，约束变更范围，支持中断恢复、任务重试，并在具备实际收益时启用 Codex 原生多智能体协作，帮助 Codex 更可靠地完成跨模块、长周期和高风险工程任务。

工作流通过明确的用户确认门禁控制计划到执行的转换，并根据任务风险提供范围校验、失败阻断、验证和条件式回滚检查。常规小改动、格式化和简单 CRUD 通常不需要使用本插件。
...
方案汇报先用一句话说明：按 Project Workflow 的什么流程，为哪个任务完成了哪些规划成果。随后展示真实文档链接，每个链接单独成段并加粗，链接文字使用当前会话语言的用途标签，冒号后保留实际文件名及扩展名。轻量流程若将设计合并在计划中，只链接已有计划文件。

链接后用最多三条简短内容说明范围与实施方式，再单独说明规划验证结果及适用的执行、恢复事项。最后只提出一次确认请求，明确任务名称和计划版本。避免手动添加文件图标、装饰分隔线、重复确认，以及宣称尚未完成的实现或测试。

首句、链接标签、摘要和确认提示均随当前对话语言与实际任务调整，约束的是语义与结构，不固定使用中文或英文句式；实际文件名和机器标识保持不变。内部任务状态展示仍支持 `zh-CN` 和 `en-US`，其他语言回退英语。既有文档生成目录约定不变。
